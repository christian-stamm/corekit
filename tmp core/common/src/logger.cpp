...
namespace corekit {
    ...
    std::streambuf::int_type StreamBuf::underflow() {
        if (device == nullptr || !device->isLoaded())
            return traits_type::eof();
    ...
    std::streamsize StreamBuf::xsputn(const char* s, std::streamsize count) {
        if (device != nullptr && device->isLoaded() && 0 < count) {
    ...
    std::streamsize StreamBuf::xsgetn(char* s, std::streamsize count) {
        if (!device || !device->isLoaded() || count <= 0)
            return 0;
    ...
    std::string Logger::stamp2string(bool precise) {
        const uint64_t total_ns = static_cast<uint64_t>(1e9 * Time::now());

        const uint64_t ns   = (total_ns % 1'000'000'000ULL);
        const uint64_t secs = (total_ns / 1'000'000'000ULL) % 60ULL;
        const uint64_t mins = (total_ns / 1'000'000'000ULL / 60ULL) % 60ULL;
        const uint64_t hrs  = (total_ns / 1'000'000'000ULL / 3600ULL) % 24ULL;

        return std::format("{:02}:{:02}:{:02}.{:09}", hrs, mins, secs, ns);
    }
