...
namespace mcap::internal {
    ...
    class IntervalTree {
        public:
            ...
            std::unique_ptr<IntervalTree> clone() const { return std::unique_ptr<IntervalTree>(new IntervalTree(*this)); }
            ...
            IntervalTree(interval_vector&& ivals, std::size_t depth = 16, std::size_t minbucket = 64, std::size_t maxbucket = 512, Scalar leftextent = 0, Scalar rightextent = 0)
                ...
                if (!ivals.empty()) { center = (minmaxStart.first->start + minmaxStop.second->stop) / 2; }
                ...
                if (depth == 0 || (ivals.size() < minbucket && ivals.size() < maxbucket)) {
                    ...
                    if (!lefts.empty()) { left.reset(new IntervalTree(std::move(lefts), depth, minbucket, maxbucket, leftp, center)); }
                    if (!rights.empty()) { right.reset(new IntervalTree(std::move(rights), depth, minbucket, maxbucket, center, rightp)); }
                }
                assert(is_valid().first);
            }
            ...
            void visit_contained(const Scalar& start, const Scalar& stop, UnaryFunction f) const {
                auto filterF = [&](const interval& cur) {
                    if (start <= cur.start && cur.stop <= stop) { f(cur); }
                };
                visit_near(start, stop, filterF);
            }
            ...
            std::pair<bool, std::pair<Scalar, Scalar>> is_valid() const {
                ...
                if (!std::is_sorted(intervals.begin(), intervals.end(), IntervalStartCmp())) { result.first = false; }
                return result;
            }
