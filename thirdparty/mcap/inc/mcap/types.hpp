 ...
 */
    struct MCAP_PUBLIC Record {
            ...
            uint64_t recordSize() const { return sizeof(opcode) + sizeof(dataSize) + dataSize; }
    };
 ...
 */
    struct MCAP_PUBLIC Schema {
            ...
            Schema(const std::string_view _name, const std::string_view _encoding, const std::string_view _data)
                : name(_name)
                , encoding(_encoding)
                , data{reinterpret_cast<const std::byte*>(_data.data()), reinterpret_cast<const std::byte*>(_data.data() + _data.size())} { }
 ...
 */
    struct MCAP_PUBLIC Channel {
            ...
            Channel(const std::string_view _topic, const std::string_view _messageEncoding, SchemaId _schemaId, const KeyValueMap& _metadata = {})
                : topic(_topic)
 ...
 */
    ...
    struct MCAP_PUBLIC RecordOffset {
            ...
            bool operator!=(const RecordOffset& other) const { return !(*this == other); }

            bool operator>=(const RecordOffset& other) const { return ((*this == other) || (*this > other)); }
 ...
 */
    struct MCAP_PUBLIC MessageView {
            ...
            MessageView(const Message& _message, const ChannelPtr _channel, const SchemaPtr _schema, RecordOffset offset)
                : message(_message)
                , channel(_channel)
                , schema(_schema)
