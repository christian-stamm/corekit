...
static void wizchip_write_buf(uint8_t *tx_data, datasize_t len) { spi_write_blocking(SPI_PORT, tx_data, len); }
#    endif
...
static void wizchip_critical_section_lock(void) { critical_section_enter_blocking(&g_wizchip_cri_sec); }
...
void print_ipv6_addr(uint8_t *name, uint8_t *ip6addr) {
    ...
    printf(
        "%04X:%04X",
        ((uint16_t)ip6addr[0] << 8) | ((uint16_t)ip6addr[1]),
        ((uint16_t)ip6addr[2] << 8) | ((uint16_t)ip6addr[3]));
    printf(
        ":%04X:%04X",
        ((uint16_t)ip6addr[4] << 8) | ((uint16_t)ip6addr[5]),
        ((uint16_t)ip6addr[6] << 8) | ((uint16_t)ip6addr[7]));
    printf(
        ":%04X:%04X",
        ((uint16_t)ip6addr[8] << 8) | ((uint16_t)ip6addr[9]),
