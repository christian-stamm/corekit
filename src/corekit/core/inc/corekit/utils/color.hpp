...
namespace corekit {
    namespace utils {
        ...
        struct Color {
            using List = std::vector<Color>;

            // HSV (h in [0,360), s,v in [0,1]) to RGB (r,g,b in [0,1])
            static Color hsv2rgba(float h, float s, float v, float a = 1.0f) {
                h = std::fmod(std::fmod(h, 360.0f) + 360.0f,
                              360.0f);  // wrap hue
            ...
            static Color::List sample(int N) {
                Color::List colors;
