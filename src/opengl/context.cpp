...
namespace opengl {
    ...
    void glCheckError(std::string meta, const std::source_location& location)
    {
        const GLenum& errcode = glGetError();

        corecheck(
            errcode == GL_NO_ERROR, std::format("OpenGL Code: 0x{:04X} ({:08}) {}", errcode, errcode, meta), location);
    }
