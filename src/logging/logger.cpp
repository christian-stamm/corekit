...
namespace logging {
    ...
    {
        return std::format("[{}][{}][{:<{}}] ", level2string(level), stamp2string(), name, NAME_SIZE);
    }

    std::string Logger::name2string(const std::string& name)
    {
        std::string tf = name.substr(0, NAME_SIZE);
        std::transform(tf.begin(), tf.end(), tf.begin(), [](unsigned char c) { return std::toupper(c); });
        return tf;
    }
    ...
    std::string Logger::stamp2string(bool precise)
    {
        ...
        uint32_t h  = std::chrono::duration_cast<std::chrono::hours>(t).count() % 24;
        uint32_t m  = std::chrono::duration_cast<std::chrono::minutes>(t).count() % 60;
        uint32_t s  = std::chrono::duration_cast<std::chrono::seconds>(t).count() % 60;
        uint32_t us = std::chrono::duration_cast<std::chrono::microseconds>(t).count() % 1000000;
        return std::format("{:02}:{:02}:{:02}", h, m, s) + (precise ? std::format(":{:06}", us) : "");
    }
