...
namespace device {
    ...
    Device::~Device()
    {
        // Destructor must not throw. Ensure we attempt to unload but swallow
        // any exceptions from cleanup.
        try {
            unload();
        }
        catch (...) {
            logger(Level::ERROR) << ("Exception thrown during device cleanup in destructor");
        }
    }
    ...
    bool Device::unload()
    {
        bool expected = true;
        bool desired  = false;
        // Transition from loaded -> not-loaded once. The thread that wins runs cleanup().
        if (loaded.compare_exchange_strong(expected, desired)) {
            try {
                return cleanup();
            }
