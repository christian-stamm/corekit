...
namespace opengl {
    ...
    class Texture : public Device {
      public:
        ...
        void bind() const;
        void unbind() const;
        void fill(cv::Mat image, uint layer = 0, FillMode mode = RESIZE_IMAGE);
        void copyTo(const Ptr& target, GLenum mask = GL_COLOR_BUFFER_BIT, GLuint filter = GL_NEAREST) const;
