...
#include "corekit/device/module/mouse.hpp"
...
namespace nlohmann {
    ...
    using namespace corekit::render::opengl;

    inline void from_json(const ordered_json& j, Window::Settings& s) {
        j.at("title").get_to(s.title);
        j.at("shape").get_to(s.shape);
        j.at("visible").get_to(s.visible);
    }

    inline void to_json(ordered_json& j, const Window::Settings& s) {
        j = ordered_json{
            {"title", s.title},
            {"shape", s.shape},
            {"visible", s.visible},
