...
namespace corekit::Gpio {
    ...
    Range Range::slice(int shift, int length) const {
        ...
        core::check(
            ...
            OutOfRangeError(std::format(
                "Slice out of range: base={} " "length={} shift={} slice_length={}",
                this->base,
                this->length,
                shift,
                length)));
    ...
    bool setChannelIRQ(uint pin, uint32_t events, Handle callback) {
        core::check(
            pin < NUM_BANK0_GPIOS,
            OutOfRangeError(std::format("Invalid GPIO pin: {} (max={})", pin, NUM_BANK0_GPIOS - 1)));
    ...
    bool configure(
        ...
        core::check(
            !(pullUp && pullDown),
            InvalidArgumentError(std::format("Invalid GPIO function: pullUp and pullDown cannot both be true")));
