...
namespace corekit::Dma {
    ...
    bool IsrDaemon::configure(uint channel, Handle handle) {
        core::check(
            channel < NUM_DMA_CHANNELS,
            OutOfRangeError(std::format("Invalid DMA channel: {} (max={})", channel, NUM_DMA_CHANNELS - 1)));
    ...
    bool IsrDaemon::on_init(StopToken token) {
        enableIRQ();
    ...
    bool IsrDaemon::on_exec(StopToken token) {
    ...
    bool IsrDaemon::on_exit(StopToken token) {
        disableIRQ();
        return true;
    }
    ...
    //
    {
        ...
        if (wrap_length != 0) {
            ...
            core::check(math::isPow2(wrapBytes), InvalidArgumentError("DMA ring size must be a power of two"));

            const uint ringBits = static_cast<uint>(std::countr_zero(wrapBytes));
            core::check(
                ringBits >= MIN_RING_BITS && ringBits <= MAX_RING_BITS,
                OutOfRangeError("DMA ring size is outside the supported range"));

            const volatile void* ringBase = 0 < wrap_length ? targetAddr : originAddr;
            core::check(ringBase != nullptr, InvalidArgumentError("DMA ring base address is null"));
            core::check(
                (reinterpret_cast<uintptr_t>(ringBase) % wrapBytes) == 0,
                InvalidArgumentError("DMA ring base address is not naturally aligned"));
    ...
    bool Device::process(Transfer::Ptr task) {
        core::check(task != nullptr, InvalidArgumentError("DMA Task is null"));
        core::check(
            task->channel == channel,
            RuntimeError(std::format("DMA Task channel mismatch: task={} device={}", task->channel, channel)));
    ...
    bool Device::restart() const {
        core::check(current_task != nullptr, RuntimeError("No current task for channel"));
    ...
    bool Device::write(const uint32_t& value) {
        return core::check(false, NotImplementedError("DMA write operation is not implemented"));
    }

    bool Device::read(uint32_t& value) {
        return core::check(false, NotImplementedError("DMA read operation is not implemented"));
    }
