...
namespace corekit::Pio {
    ...
    int Program::install(PIO block, uint node) {
        ...
        if (!isInstalled(block)) {
            core::check(
                pio_can_add_program(block, this),
                RuntimeError("Cannot install PIO program: not enough space left"));
    ...
    bool Program::modify(PIO block, uint line, Command command) {
        core::check(isInstalled(block), RuntimeError("Cannot modify a PIO program that is not installed."));
        ...
        core::check(
            line < length,
            OutOfRangeError(std::format(
                "Cannot modify a PIO program line that is out of range: " "line={} length={}",
                line,
                length)));
    ...
    bool Node::on_load() {
        core::check(program != nullptr, RuntimeError("Cannot load a PIO node without a program."));

        const int base = program->install(block, node);

        core::check(base >= 0, RuntimeError("Failed to install PIO program for node: " + name));

        NodeConf node_cfg = pio_get_default_sm_config();

        core::check(build_node_conf(node_cfg, base), RuntimeError("Failed to build PIO node configuration."));

        LaunchConf launch_cfg;

        core::check(build_launch_conf(launch_cfg), RuntimeError("Failed to build PIO launch configuration."));

        const uint initial_pc = base + launch_cfg.entrypoint;

        core::check(
            pio_sm_init(block, node, initial_pc, &node_cfg) == PICO_OK,
        ...
        core::check(
            configure_regs(pio_x, launch_cfg.scratchX),
            RuntimeError("Failed to preload PIO scratch X register."));

        core::check(
            configure_regs(pio_y, launch_cfg.scratchY),
            RuntimeError("Failed to preload PIO scratch Y register."));

        core::check(
            configure_regs(pio_isr, launch_cfg.isr),
            RuntimeError("Failed to preload PIO input shift register."));

        core::check(
            configure_regs(pio_osr, launch_cfg.osr),
            RuntimeError("Failed to preload PIO output shift register."));

        core::check(configure_dmas(), RuntimeError("Failed to configure PIO DMAs."));
    ...
    bool Node::configure_regs(pio_src_dest reg, const PreloadVal& val) {
        ...
        core::check(
            !is_running(),
            RuntimeError("Cannot preload a PIO register while the state machine " "is running."));
    ...
    bool Node::configure_pins(const PinoutCfg& config, bool is_output) {
        ...
        core::check(
            ...
            RuntimeError(std::format(
                "Cannot configure PIO pins: GPIO base is too high: " "base={} pin={}",
                base,
                pins.lower())));
        ...
        core::check(
            pins.upper() <= base + 32,
            RuntimeError(
                std::format("Cannot configure PIO pins: GPIO base is too low: " "base={} pin={}", base, pins.upper())));
    ...
    bool Node::write(const uint32_t& data) {
        core::check(is_loaded(), RuntimeError("PIO node is not loaded"));
