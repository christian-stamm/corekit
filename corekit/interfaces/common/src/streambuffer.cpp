...
namespace corekit {
    ...
    std::streambuf::int_type StreamBuffer::overflow(std::streambuf::int_type c) {
    ...
    std::streambuf::int_type StreamBuffer::underflow() {
        if (device == nullptr) { return traits_type::eof(); }

        uint8_t data;

        if (!device->read(data)) { return traits_type::eof(); }
    ...
    std::streamsize StreamBuffer::xsputn(const char* s, std::streamsize count) {
        if (0 < count && device != nullptr) {
            if (device->write_burst(std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(s), static_cast<std::size_t>(count)))) { return count; }
        }
