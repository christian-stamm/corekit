...
namespace corekit {
    ...
    class Memory : public std::span<T> {
        public:
            ...
            Ptr static request(size_t size, bool aligned = true) { return std::make_shared<Memory>(size, aligned); }

            bool isAligned() const { return isAligned<T>(base, std::span<T>::size()); }
        ...
        private:
            ...
            Memory(T* base, size_t size)
                : std::span<T>(base, size)
                , base(base) { }

            T* mem_alloc(size_t size, bool aligned) {
                size    *= sizeof(T);
                aligned &= math::isPow2(size);
                return static_cast<T*>(aligned ? aligned_alloc(size, size) : malloc(size));
            }
