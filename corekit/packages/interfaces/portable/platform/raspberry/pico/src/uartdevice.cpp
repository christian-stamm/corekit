...
namespace corekit::Uart {
    ...
    bool Device::on_load() {
        ...
        Gpio::configure(txPin, false, false, GPIO_OUT, GPIO_OVERRIDE_NORMAL, GPIO_FUNC_UART);
        Gpio::configure(rxPin, false, false, GPIO_IN, GPIO_OVERRIDE_NORMAL, GPIO_FUNC_UART);
        return true;
    }
    ...
    bool Device::write_burst(std::span<const uint8_t> data) {
        uart_write_blocking(instance, data.data(), static_cast<size_t>(data.size()));
        return true;
    }
    ...
    bool Device::read_burst(std::span<uint8_t> data) {
        uart_read_blocking(instance, data.data(), static_cast<size_t>(data.size()));
