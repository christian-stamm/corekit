...
namespace corekit {
    ...
    template <typename T>
    bool SerialDevice<T>::xferBulk(std::span<const T> txData, std::span<T> rxData) {
        const uint num_cycles = txData.size();

        if (rxData.size() < num_cycles) { return false; }

        for (size_t cycle = 0; cycle < num_cycles; ++cycle) {
            if (!xfer(txData[cycle], rxData[cycle])) { return false; }
        }
        return true;
    }
